...
def get_prompt(curr, tmpl, assessment_type, strand, typed_text):
    # Differentiate between Scaffolding (Assignments) and Evaluation (Unit Tests)
    scaffold_instruction = f"Scaffolding steps: {' -> '.join(curr.get('scaffolding_steps', ['Review concepts', 'Practice', 'Apply']))}" if assessment_type == "assignment" else ""
    
    return f"""
    You are an expert SHS Teacher at Sagkahan National High School.
    Analyze the student's work for competency: {curr['learning_competency']}
    Assessment Type: {assessment_type.upper()}
    {scaffold_instruction}

    Generate remediation/content in official DepEd DLL format:
    1. OBJECTIVES: {tmpl['I_OBJECTIVES']}
    2. PROCEDURES (Mastery): {tmpl['IV_PROCEDURES']['Mastery']}
    3. EVALUATION: {tmpl['IV_PROCEDURES']['Evaluation']}
    
    Student Typed Response: {typed_text}
...
def main():
    ...
    for index, row in enumerate(sheet.get_all_records(), start=2):
        ...
        contents = [get_prompt(curr, dll_tmpl, assessment_type, row.get("Strand_Focus"), typed_text)]
        
        # If image exists, download and append
        if file_id:
            try:
                fh = io.BytesIO()
